"""
Random Forest Regressor on the Iris Regression Task.

Predict petal_length (cm) from sepal_length, sepal_width, petal_width.

Outputs:
  outputs/figures/iris_rf_actual_vs_predicted.png
  outputs/figures/iris_rf_residuals_scatter.png
  outputs/figures/iris_rf_residuals_hist.png
  outputs/figures/iris_rf_cv_scores.png
  outputs/figures/iris_rf_feature_importance.png
  outputs/figures/iris_rf_trees_sample.png
  outputs/csv/iris_rf_metrics.csv
"""
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # 3. Train Random Forest (no scaling needed for RF)
    # ------------------------------------------------------------------
    ...
    cv_model = RandomForestRegressor(n_estimators=200, max_depth=None, random_state=42, n_jobs=-1)
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_fig / "iris_rf_actual_vs_predicted.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title("Random Forest — Residuals vs Predicted", fontsize=13)
    fig.tight_layout()
    fig.savefig(out_fig / "iris_rf_residuals_scatter.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    ax.set_title("Random Forest — Residual Distribution", fontsize=13)
    fig.tight_layout()
    fig.savefig(out_fig / "iris_rf_residuals_hist.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_fig / "iris_rf_cv_scores.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # (e) Feature importances (horizontal bar chart)
    ...
    fig.tight_layout()
    fig.savefig(out_fig / "iris_rf_feature_importance.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # (f) First 3 individual tree predictions vs actual
    tree_colors = ["#E74C3C", "#C0392B", "#922B21"]
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.scatter(y_test.values, y_pred, alpha=0.5, s=40, color="grey", label="Ensemble", zorder=2)
    for i in range(min(3, len(model.estimators_))):
        tree_pred = model.estimators_[i].predict(X_test.values)
        ax.scatter(y_test.values, tree_pred, alpha=0.4, s=20,
                   color=tree_colors[i], label=f"Tree {i+1}", zorder=3)
    ax.plot([vmin, vmax], [vmin, vmax], "k--", linewidth=1.5, label="Perfect fit", zorder=4)
    ax.set_xlabel("Actual petal_length (cm)", fontsize=12)
    ax.set_ylabel("Predicted petal_length (cm)", fontsize=12)
    ax.set_title("Random Forest — Sample Individual Trees vs Ensemble", fontsize=13)
    ax.legend(fontsize=9, loc="upper left")
    fig.tight_layout()
    fig.savefig(out_fig / "iris_rf_trees_sample.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # ------------------------------------------------------------------
    # 7. Save metrics CSV
    # ------------------------------------------------------------------
    metrics_df = pd.DataFrame([{"Model": "Random Forest", **metrics,
                                  "CV_R2_mean": cv_scores.mean(), "CV_R2_std": cv_scores.std()}])
    metrics_df.to_csv(out_csv / "iris_rf_metrics.csv", index=False)

    # ------------------------------------------------------------------
    # 8. Print summary
    # ------------------------------------------------------------------
    print("\n" + "=" * 60)
    print("RANDOM FOREST — IRIS REGRESSION SUMMARY")
    print("=" * 60)
    ...
    print(f"Saved metrics → {out_csv / 'iris_rf_metrics.csv'}")
